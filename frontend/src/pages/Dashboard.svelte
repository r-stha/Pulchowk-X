<script lang="ts">
	...
	let enrollmentLoading = $state(true);
	let enrollment = $state<Registration | null>(null);
	...
	async function loadEnrollment() {
		...
		try {
			...
			if (result.success && result.registration) {
				enrollment = result.registration;
			} else {
				enrollment = null;
			}
		} catch (err: any) {
			...
			<div class="space-y-6">
				...
				>
					...
					<div class="p-6">
						...
						{:else if enrollment}
							<a
								href="/clubs/{enrollment.event
									?.clubId}/events/{enrollment.eventId}"
								use:route
