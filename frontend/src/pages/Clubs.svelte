	...
	async function loadClubs() {
		...
		try {
			...
			if (result.success && result.existingClub) {
				clubs = result.existingClub;
			} else {
				error = result.message || "Failed to load clubs";
			}
		} catch (err: any) {
	...
	<div class="max-w-7xl mx-auto">
		...
		<div class="text-center mb-12 animate-fade-in">
			...
			<p class="text-gray-600 text-lg max-w-2xl mx-auto">
				Explore the vibrant clubs at Pulchowk Campus. Join events,
				connect with peers, and make the most of your campus life.
			</p>
		</div>
