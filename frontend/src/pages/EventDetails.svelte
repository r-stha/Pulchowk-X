<script lang="ts">
    ...
    interface Props {
        route?: { params: { clubId: string; eventId: string } };
    }
    ...
    async function loadEventDetails() {
        ...
        try {
            ...
            if (isClubOwner) {
                const studentsResult = await getRegisteredStudents(
                    parseInt(eventId),
                );
    ...
    async function checkRegistrationStatus() {
        ...
        try {
            ...
            if (result.success && result.registration) {
                isRegistered =
                    result.registration.eventId === parseInt(eventId);
            }
        } catch (err) {
...
<div class="min-h-[calc(100vh-4rem)] bg-gray-50/30 px-4 py-8 sm:px-6 lg:px-8">
    <div class="max-w-5xl mx-auto">
        ...
        {:else if event}
            <!-- Event Banner -->
            <div
                class="h-64 sm:h-80 bg-gradient-to-br from-blue-500 to-purple-600 rounded-3xl overflow-hidden mb-8 relative shadow-xl"
                                        ...
                                        <tbody class="divide-y divide-gray-50">
                                            {#each registeredStudents as reg}
                                                ...
                                                >
                                                    ...
                                                    <td
                                                        class="py-3 px-4 text-sm text-gray-500"
                                                    >
                                                        {new Date(
                                                            reg.registeredAt,
                                                        ).toLocaleDateString()}
                    ...
                    {#if !isClubOwner}
                        ...
                        >
                            ...
                            {:else}
                                ...
                                >
                                    Clicking register will reserve your spot
                                    immediately.
                                </p>
                            {/if}
                        </div>
