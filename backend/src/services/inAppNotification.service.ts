...
async function enrichNotificationData(
  ...
  const lostFoundItemIds = new Set<number>()
  ...
  for (const row of rows) {
    ...
    const listingId =
      parseNumericId(data.listingId) ?? parseNumericId(data.bookId)
    ...
    if (
      itemId &&
      (row.type.startsWith('lost_found') ||
        parseString(data.iconKey) === 'lost_found' ||
        parseString(data.iconKey) === 'search')
    ) {
      lostFoundItemIds.add(itemId)
    }
  ...
  const [
    ...
    requestIds.size > 0
      ? db
          ...
          .innerJoin(
            bookListings,
            eq(bookListings.id, bookPurchaseRequests.listingId),
          )
    ...
    claimIds.size > 0
      ? db
          ...
          .innerJoin(
            lostFoundItems,
            eq(lostFoundItems.id, lostFoundClaims.itemId),
          )
    ...
    lostFoundItemIds.size > 0
      ? db
          ...
          .where(inArray(lostFoundImages.itemId, [...lostFoundItemIds]))
          .orderBy(sql`${lostFoundImages.sortOrder} asc`)
      : Promise.resolve([]),
  ])

  const eventMap = new Map<
    number,
    { bannerUrl: string | null; title: string | null }
  >(
    eventRows.map((row) => [
      row.id,
      {
  ...
  const noticeMap = new Map<
    number,
    { attachmentUrl: string | null; title: string | null }
  >(
    noticeRows.map((row) => [
      row.id,
      {
  ...
  const lostFoundImageMap = new Map<number, string>()
  for (const row of lostFoundImageRows) {
    if (!lostFoundImageMap.has(row.itemId)) {
      lostFoundImageMap.set(row.itemId, row.imageUrl)
    }
  }
  ...
  return rows.map((row) => {
    ...
    if (!existingThumb) {
      ...
      if (eventMeta?.bannerUrl) data.thumbnailUrl = eventMeta.bannerUrl
        ...
        noticeMeta?.attachmentUrl &&
        isImageUrl(noticeMeta.attachmentUrl)
      )
        data.thumbnailUrl = noticeMeta.attachmentUrl
      else if (listingMeta?.imageUrl) data.thumbnailUrl = listingMeta.imageUrl
      else {
        const lfItemId = parseNumericId(data.itemId)
        const lfImage = lfItemId ? lostFoundImageMap.get(lfItemId) : null
        if (lfImage) data.thumbnailUrl = lfImage
      }
    }
...
function getPreferenceTypeFilter(preferences: NotificationPreferences) {
  ...
  if (!preferences.chatAlerts) {
    filters.push(
      sql`not (${notifications.type} in ('chat_message', 'chat_mention'))`,
    )
  }
...
export async function createInAppNotificationForUser(input: {
  ...
  if (
    !isNotificationTypeAllowedByPreferences(input.type, prefs.data, input.data)
  ) {
    return null
  }
